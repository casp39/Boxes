...
public class RoomPinkScript : MonoBehaviour {
	...
	void Update () {
		...
		if ((315 < cameraRotY  && cameraRotY < 360) || (cameraRotY <= 45)) {
			if ((Input.GetKey(KeyCode.Space) && Input.GetKey(KeyCode.RightArrow)) && room_active == true) {
				rigidbody.velocity = Vector3.right * room_speed;
			} else if ((Input.GetKey(KeyCode.Space) && Input.GetKey(KeyCode.LeftArrow)) && room_active == true) {
				rigidbody.velocity = Vector3.left * room_speed;
			} else if ((Input.GetKey(KeyCode.Space) && Input.GetKey(KeyCode.UpArrow)) && room_active == true) {
